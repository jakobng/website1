...
def scrape_shin_bungeiza() -> List[Dict]:
    ...
    # Define the date range for scraping: today and the next 7 days.
    today = _dt.date.today()
    end_date = today + _dt.timedelta(days=7)
    print(f"INFO: [{CINEMA_NAME_SB}] Filtering for dates between {today} and {end_date}.")
    ...
    for box in program_blocks:
        ...
        for date_header in content_div.select('h2'):
            ...
            # Use the 'today' variable defined outside the loop for efficiency.
            year = today.year if month >= today.month else today.year + 1
            
            try:
                show_date = _dt.date(year, month, day)
            except ValueError:
                continue # Skip invalid dates

            # Only process dates within our desired range.
