...
---
v7 Final:
* Fixes inconsistent movie IDs (e.g., '12315' vs '1231500') by padding them.
* Fixes bug where ratings (e.g., "R15+") were included in the 'country' field.
* Handles 404 Not Found errors for special events (e.g., stage greetings).
* When a detail page is missing, it now falls back to using the 'running_time'
  from the main schedule JSON, in addition to the title.
"""
...
# Regex to match and filter out Japanese film ratings
_RATING_RE = re.compile(r"^(R18|R15|PG12|G)\+?$", re.IGNORECASE)
...
def _scrape_detail_page(movie_id: str) -> Dict[str, Optional[str]]:
    """
    Scrapes the movie detail page for rich metadata.
    Returns an empty dict if the page gives a 404 or other error.
    """
    if movie_id in _detail_cache:
        return _detail_cache[movie_id]

    # **IMPROVEMENT**: Fix for inconsistent movie IDs from the data feed.
    # The feed sometimes provides a shortened ID (e.g., '12315') while the
    # actual URL requires the full 7-digit ID ('1231500').
    corrected_movie_id = movie_id.ljust(7, '0')
    url = DETAIL_URL_TEMPLATE.format(movie_id=corrected_movie_id)

    try:
        soup = BeautifulSoup(_get(url).text, "html.parser")
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 404:
            print(f"[WARN] No detail page for '{movie_id}' (tried {url}). Using fallback data.", file=sys.stderr)
        else:
            print(f"[WARN] detail page {url}: {e}", file=sys.stderr)
        _detail_cache[movie_id] = {} # Cache failure to prevent retries
        return {}
    except Exception as e:
        print(f"[WARN] detail page {url}: {e}", file=sys.stderr)
    ...
    # --- Year Extraction (Robust Fallbacks) ---
    year = None
    page_text_for_year = soup.get_text(" ", strip=True)
    ...
    if copyright_tag and (m := copyright_regex.search(copyright_tag.text)):
        year = m.group(1)
    elif (m := copyright_regex.search(page_text_for_year)):
        year = m.group(1)
    elif (m := re.search(r"（(\d{4})年製作）", page_text_for_year)):
        year = m.group(1)

    # --- Runtime, Country, and Year (from metadata line) ---
    runtime_min = country = None
    label = soup.select_one("p.schedule-nowShowing-label")
    if label:
        parts = [b.get_text(strip=True) for b in label.select("b") if b.get_text(strip=True)]
        remaining_parts = []
        for part in parts:
            if "分" in part and part[:-1].isdigit():
                runtime_min = part[:-1]
            elif part.endswith('年') and part[:-1].isdigit() and not year:
                year = part[:-1]
            # **IMPROVEMENT**: Filter out film rating codes
            elif not _RATING_RE.match(part):
                remaining_parts.append(part)

        if remaining_parts:
            potential_country = "・".join(remaining_parts)
            if "配給" not in potential_country:
                country = potential_country

    # --- Director Extraction ---
    director = None
    for dt_tag in soup.select("dl.movie-staff dt"):
        if "監督" in dt_tag.get_text():
            dd = dt_tag.find_next_sibling("dd")
            if dd:
                director = dd.get_text(" ", strip=True).lstrip("：:").strip()
            break
...
def scrape_human_shibuya(max_days: int = 7) -> List[Dict]:
        ...
        for mid in map(str, d.get("movie", [])):
            ...
            for scr in screens.get(key, []):
                ...
                for t in scr.get("time", []):
                    ...
                    # Attempt to get rich metadata from the movie's detail page
                    meta = _scrape_detail_page(mid)

                    # If detail page scraping fails, use basic info from the schedule feed
                    if not meta.get("detail_page_url"):
                        mv_data_list = movies_map.get(mid)
                        mv = {}
                        if isinstance(mv_data_list, list) and mv_data_list:
                            current_date_str = f"{y}{_zfill(m)}{_zfill(day)}"
                            mv = next((m for m in mv_data_list if f"{m.get('date_year')}{_zfill(m.get('date_month'))}{_zfill(m.get('date_day'))}" == current_date_str), mv_data_list[0])
                        
                        if isinstance(mv, dict):
                            meta["movie_title"] = mv.get("name") or mv.get("cname")
                            meta["runtime_min"] = mv.get("running_time")
    ...
    unique = [dict(t) for t in {tuple(d.items()) for d in result}]
