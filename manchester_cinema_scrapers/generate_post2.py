"""Generate Instagram-ready image carousel (V2 - \"A24 Style\" + Spread Layout) for Manchester.
...
- Update: Enforces Manchester timezone, 3-day film rotation history.
...
# --- Timezone: Manchester ---
MANCHESTER_TZ = ZoneInfo("Europe/London")

def get_today_manchester():
    """Returns the current datetime in Manchester."""
    return datetime.now(MANCHESTER_TZ)

def get_today_str():
    """Returns YYYY-MM-DD in Manchester."""
    return get_today_manchester().strftime("%Y-%m-%d")

def get_display_date_str():
    """Returns formatted date string for display (Manchester)."""
    d = get_today_manchester()
...
class HistoryManager:
    ...
    def is_on_cooldown(self, film_id):
        ...
        today = get_today_manchester().date()
...
def main():
    from PIL import ImageEnhance
    print("--- Starting Manchester Today's Film Selection (V2) ---")
    ...
    today_dt = get_today_manchester().date()
    date_strs = [
    ...
    caption_lines = [f"🎬 Manchester Today's Film Selection - {get_display_date_str()}\n"]
