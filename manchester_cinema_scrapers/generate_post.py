"""
Generate Instagram-ready image carousel (Manchester Edition - Based on London V2.2).
"""
...
# --- Timezone: Manchester (UK - UTC+0/+1 depending on DST) ---
def today_in_manchester() -> datetime:
    """Returns Manchester datetime."""
    try:
...
# --- Database (Manchester Cinemas) ---
CINEMA_ADDRESSES = {
    # Manchester cinemas
    "HOME Manchester": "2 Tony Wilson Place, Manchester M15 4FN",
    "Cultplex": "Unit 7, Hallam Mill, Hallam St, Stockport SK2 6PT",
    "The Savoy": "Heaton Moor Rd, Stockport SK4 4HY",
    "Everyman Manchester": "Deansgate Sq, Owen St, Manchester M15 4FN",
    "Regent Cinema": "Marple Rd, Marple, Stockport SK6 7AY",
    "The Plaza": "Mersey Square, Stockport SK1 1SP",
    "The Block Cinema": "5-7 School Lane, Didsbury, Manchester M20 6RD",
    "Small World Cinema Club": "The Peer Hat, 14-16 Faraday St, Manchester M1 1BE",
}
...
def is_major_chain(cinema_name: str) -> bool:
    ...
    if "everyman" in name or "picturehouse" in name or "curzon" in name or "odeon" in name or "vue" in name:
        return True
    return False
...
def creative_director_review(original_layout: Image.Image, date_text: str) -> str:
    ...
    prompt = f"""
        ...
        Your Goal: Write a prompt for a Generative AI that will fuse these isolated elements into a SINGLE, SOPHISTICATED, IMPOSSIBLE ARCHITECTURAL STRUCTURE for a MANCHESTER cinema post.
...
def write_caption_for_multiple_cinemas(date_str: str, all_featured_cinemas: list[dict]) -> None:
    header = f"🎬 Manchester Cinema Showtimes ({date_str})\n"
    lines = [header]
    ...
    footer = f"""
#ManchesterCinema #{dynamic_hashtag} #IndependentCinema #FilmListings
...
def main() -> None:
    # 1. Basic Setup
    today = today_in_manchester().date()
    ...
    print(f"🕒 Generator Time (Manchester): {today} (String: {today_str})")
    ...
    # Hero Image Generation (Gemini-based workflow)
    if GEMINI_API_KEY:
        try:
        ...
        print("   ⚠️ GEMINI_API_KEY missing. Skipping Hero.")
