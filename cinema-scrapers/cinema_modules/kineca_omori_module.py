"""
kineca_omori_module.py — scraper for キネカ大森 (Kineca Omori)

Part of TTCG (テアトルシネマグループ), uses similar JS data structure.
Located in Omori (Shinagawa-ku), Japan's first cineplex since 1984.
Offers both roadshow and revival screenings.
"""
...
SCHEDULE_DATA_URL = f"{BASE_URL}/data/cineka_omori.js"
...
def _fetch_content(url: str, is_json: bool) -> Optional[str]:
    ...
    try:
        ...
        print(f"INFO: [{CINEMA_NAME}] Fetching {'JSON' if is_json else 'HTML'} from {request_url}")
...
def _fix_mojibake(text: Optional[str]) -> Optional[str]:
    """
    Attempt to repair common UTF-8→Latin-1 mojibake such as 'ãã¼ãã...'.
    """
    if text is None:
        return None
...
# Typical Shift-JIS→UTF-8 mojibake characters
MOJIBAKE_MARKERS = (
...
def _parse_js_variable(js_content: str) -> Optional[Any]:
    ...
    try:
        ...
        print(f"ERROR: [{CINEMA_NAME}] Could not parse JSON from JS content: {e}", file=sys.stderr)
        return None
...
def scrape_kineca_omori(max_days: int = 7) -> List[Dict[str, Any]]:
    """
    Scrape the Kineca Omori schedule.
    ...
    if not schedule_data:
        print(f"ERROR: [{CINEMA_NAME}] Failed to fetch or parse schedule data. Aborting.")
        return []
    ...
    for date_info in dates_to_process:
        ...
        for movie_id in date_info.get("movie", []):
            ...
            if re.search(r"トーク|舞台挨拶|予告編", json_title):
                print(f"INFO: [{CINEMA_NAME}] Skipping likely event: '{json_title}'")
                continue

            detail_page_url = urljoin(
                BASE_URL, f"cineka_omori/movie/{movie_id_str}.html"
            )
            ...
            # Drop remaining mojibake blocks that cannot be repaired.
            if _looks_shiftjis_mojibake(clean_title):
                print(
                    f"INFO: [{CINEMA_NAME}] Skipping mojibake block: "
...
if __name__ == "__main__":
    ...
    showings = scrape_kineca_omori(max_days=7)

    if showings:
        output_filename = "kineca_omori_showtimes.json"
