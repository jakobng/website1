#!/usr/bin/env python3
# everyman_hampstead_module.py
# Scraper for Everyman Hampstead (X06ZW)
...
CINEMA_NAME = "Everyman Hampstead"
THEATER_ID = "X06ZW"
...
def _fetch_scheduled_movie_ids(session: requests.Session) -> List[str]:
...
def _fetch_schedule(session: requests.Session, start: dt.datetime, end: dt.datetime) -> Dict:
...
def scrape_everyman_hampstead() -> List[Dict]:
    """
    Scrape Everyman Hampstead showtimes using the boxoffice API.
    """
    shows: List[Dict] = []
    ...
    unique_shows = []
    for show in shows:
        key = (show["movie_title"], show["date_text"], show["showtime"], show["booking_url"])
        if key not in seen:
            seen.add(key)
...
if __name__ == "__main__":
    data = scrape_everyman_hampstead()
