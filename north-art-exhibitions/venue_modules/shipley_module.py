...
BASE_URL = "https://shipleyartgallery.org.uk"
WHATS_ON_URL = "https://shipleyartgallery.org.uk/whats-on"
...
def scrape_shipley():
    ...
    try:
        ...
        raise RuntimeError(f"Failed to fetch {WHATS_ON_URL}: {e}") from e
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not title or len(title) < 3:
            continue
        if title.lower() in ("what's on", "view all", "visit us"):
            continue
        date_text = ""
        parent = a.parent
        for _ in range(5):
