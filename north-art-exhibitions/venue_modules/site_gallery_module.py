...
BASE_URL = "https://www.sitegallery.org"
WHATSON_URL = "https://www.sitegallery.org/whats-on/"
VENUE_NAME = "Site Gallery"
VENUE_CITY = "Sheffield"
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; NorthArtExhibitions/1.0)", "Accept-Language": "en-GB,en;q=0.9"}
TIMEOUT = 25


def scrape_site_gallery():
    out = []
    try:
        ...
        raise RuntimeError("Failed to fetch Site Gallery: " + str(e)) from e
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not title or len(title) < 3:
            continue
        if title.lower() in ("exhibition", "view archive", "stay up to date"):
            continue
        date_text = ""
        parent = a.parent
        for _ in range(5):
