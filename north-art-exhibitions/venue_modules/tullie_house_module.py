...
BASE_URL = "https://tullie.org.uk"
WHATSON_URL = "https://tullie.org.uk/whats-on"
VENUE_NAME = "Tullie House Museum and Art Gallery"
VENUE_CITY = "Carlisle"
HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; NorthArtExhibitions/1.0)", "Accept-Language": "en-GB,en;q=0.9"}
TIMEOUT = 25


def scrape_tullie_house():
    out = []
    try:
        ...
        raise RuntimeError("Failed to fetch Tullie House: " + str(e)) from e
    ...
    for a in soup.find_all("a", href=True):
        ...
        if full_url.rstrip("/") == WHATSON_URL.rstrip("/"):
            continue
        title = norm(a.get_text())
        if not title or len(title) < 3:
            continue
        if title.lower() in ("what's on", "tullie", "visit"):
            continue
        date_text = ""
        parent = a.parent
        for _ in range(5):
